...
# Subir archivos de la señal (.dat y .hea)
uploaded = files.upload()
# Asegúrate de subir: drive01.dat y drive01.hea
...
def calcular_coeficiente_variacion(voltaje):
    return np.std(voltaje) / np.mean(voltaje)
...
def custom_std_dev(data):
    ...
    return (sum((x - mean) ** 2 for x in data) / len(data)) ** 0.5 if len(data) != 0 else 0.0

def custom_coeff_variation(data):
    mean = custom_mean(data)
    return (custom_std_dev(data) / mean) * 100 if mean != 0 else 0.0

signal = wfdb.rdrecord('drive01')  # Ahora solo el nombre base
...
def menu_interactivo():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print(f"SNR del ruido Gaussiano: {calcular_SNR(voltaje_canal_1, ruido_gaussiano)} dB")
            ...
            ruido_impulso = np.random.choice(
                [0.02, amplitud_impulso, -amplitud_impulso],
                size=voltaje_canal_1.shape,
                p=[1 - probabilidad_impulso, probabilidad_impulso / 2, probabilidad_impulso / 2]
            )
            voltaje_contaminado_impulso = voltaje_canal_1 + ruido_impulso
            print(f"SNR del ruido de Impulso: {calcular_SNR(voltaje_canal_1, ruido_impulso)} dB")
            ...
            print(f"SNR del ruido Artefacto: {calcular_SNR(voltaje_canal_1, ruido_artefacto)} dB")
